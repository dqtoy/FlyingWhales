...
public class Player : ILeader {
    ...
    public List<Intel> allIntel { get; }
    public List<Minion> minions { get; }
    public List<SummonSlot> summonSlots { get; }
    public List<ArtifactSlot> artifactSlots { get; }
    ...
    public bool AreAllArtifactSlotsMaxLevel() {
        for (int i = 0; i < maxArtifactSlots; i++) {
            if (artifactSlots[i].level < PlayerDB.MAX_LEVEL_ARTIFACT) {
                return false;
            }
        }
        return true;
