...
public class ReportHostile : GoapAction {
    ...
    public override void PerformActualAction() {
        ...
        if (!isTargetMissing && targetCharacter.IsInOwnParty()) {
            if(hostile.GetNumOfJobsTargettingThisCharacter(JOB_TYPE.ASSAULT) < 3 && !hostile.HasTraitOf(TRAIT_TYPE.DISABLER, "Combat Recovery")) {
    ...
    private void PreReportHostileSuccess() {
        ...
        Character targetCharacter = poiTarget as Character;
        int numOfJobs = 3 - hostile.GetNumOfJobsTargettingThisCharacter(JOB_TYPE.ASSAULT);
        if (numOfJobs > 0) {
