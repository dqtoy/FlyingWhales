...
public class ChatCharacter : GoapAction {
    ...
    public override void Perform(ActualGoapNode goapNode) {
        ...
        goapNode.actor.SetIsChatting(true);
        targetCharacter.SetIsChatting(true);
        goapNode.actor.marker.UpdateActionIcon();
        targetCharacter.marker.UpdateActionIcon();

        targetCharacter.marker.LookAt(goapNode.actor.marker.transform.position, true); //For Trailer Build Only
        ...
        dueDate.AddTicks(2);
        SchedulingManager.Instance.AddEntry(dueDate, () => goapNode.actor.EndChatCharacter(), goapNode.actor);
        SchedulingManager.Instance.AddEntry(dueDate, () => targetCharacter.EndChatCharacter(), targetCharacter);

        SetState(chatResult, goapNode);
        ...
        if (chatResult == "Become Friends") {
            ...
            FlirtWith(goapNode.actor, targetCharacter);
        ...
        Plagued actorPlagued = goapNode.actor.traitContainer.GetNormalTrait("Plagued") as Plagued;
        Plagued targetPlagued = goapNode.poiTarget.traitContainer.GetNormalTrait("Plagued") as Plagued;
        //Plagued chances
        if ((actorPlagued == null || targetPlagued == null) && (actorPlagued != null || targetPlagued != null)) {
            ...
            if (actorPlagued != null) {
                plaguedSummary += "\nPlagued character is " + goapNode.actor.name + ", will roll to infect " + goapNode.poiTarget.name;
                ...
                if (roll < actorPlagued.GetChatInfectChance()) {
                    ...
                    plaguedSummary += "\nWill infect " + goapNode.poiTarget.name + " with plague!";
                ...
                plaguedSummary += "\nPlagued character is " + goapNode.poiTarget.name + ", will roll to infect " + goapNode.actor.name;
                ...
                if (roll < targetPlagued.GetChatInfectChance()) {
                    ...
                    plaguedSummary += "\nWill infect " + goapNode.actor.name + " with plague!";
                    if (goapNode.actor.traitContainer.AddTrait(goapNode.actor, "Plagued", goapNode.poiTarget as Character)) {
                        Log log = new Log(GameManager.Instance.Today(), "Character", "NonIntel", "contracted_plague");
                        log.AddToFillers(goapNode.actor, goapNode.actor.name, LOG_IDENTIFIER.ACTIVE_CHARACTER);
    ...
    private void PreQuickChat(ActualGoapNode goapNode) {
        //GoapActionState currentState = goapNode.action.states[goapNode.currentStateName];
        //TODO: currentState.SetIntelReaction(QuickChatIntelReaction);
    }
    private void PreShareInformation(ActualGoapNode goapNode) {
        //TODO: currentState.SetIntelReaction(ShareInformationIntelReaction);
    }
    private void PreBecomeFriends(ActualGoapNode goapNode) {
        //TODO: currentState.SetIntelReaction(BecomeFriendsIntelReaction);
    }
    private void PreArgument(ActualGoapNode goapNode) {
        //TODO: currentState.SetIntelReaction(ArgumentIntelReaction);
    }
    private void AfterArgument(ActualGoapNode goapNode) {
        GoapActionState currentState = goapNode.action.states[goapNode.currentStateName];
        Character targetCharacter = goapNode.poiTarget as Character;
        if (goapNode.actor.traitContainer.GetNormalTrait("Hothead") != null) {
    ...
    private void PreFlirt(ActualGoapNode goapNode) {
        //TODO: currentState.SetIntelReaction(FlirtIntelReaction);
        Character targetCharacter = goapNode.poiTarget as Character;
        goapNode.actor.SetIsFlirting(true);
        targetCharacter.SetIsFlirting(true);
        goapNode.actor.marker.UpdateActionIcon();
        targetCharacter.marker.UpdateActionIcon();
    }
    private void PreBecomeLovers(ActualGoapNode goapNode) {
        //TODO: currentState.SetIntelReaction(BecomeLoversIntelReaction);
    }
    private void PreBecomeParamours(ActualGoapNode goapNode) {
        //TODO: currentState.SetIntelReaction(BecomeParamoursIntelReaction);
    }
    private void PreResolveEnmity(ActualGoapNode goapNode) {
