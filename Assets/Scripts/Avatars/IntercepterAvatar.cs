...
public class IntercepterAvatar : MonoBehaviour {
...
//		}
//	}
	internal void Init(Intercepter intercepter){
		this.intercepter = intercepter;
		this.direction = DIRECTION.LEFT;
		this.GetComponent<Avatar> ().kingdom = this.intercepter.citizen.city.kingdom;
		this.GetComponent<Avatar> ().gameEvent = this.intercepter.sendReliefGoods;
		this.GetComponent<Avatar> ().citizen = this.intercepter.citizen;
...
//	}
	void OnTriggerEnter2D(Collider2D other){
		if(other.tag == "Avatar"){
			if(this.gameObject != null && other.gameObject != null){
				if(other.gameObject.GetComponent<Avatar>().kingdom.id != this.intercepter.citizen.city.kingdom.id){
			...
			if(this.gameObject != null && other.gameObject != null){
				Kingdom kingdomOfGeneral = this.intercepter.citizen.city.kingdom;
...
//	}
	...
	public void IsThereCitizen(){
		if(this.intercepter.citizen != null){
	...
	public void IsThereEvent(){
		if(this.intercepter.sendReliefGoods != null){
			Task.current.Succeed ();
		}else{
			Task.current.Fail ();
		}
	...
	public void HasArrivedAtTargetHextile(){
		if(this.intercepter.location == this.intercepter.targetLocation){
			if(!this.hasArrived){
				this.hasArrived = true;
				this.intercepter.Attack ();
			}
			Task.current.Succeed ();
		}else{
			Task.current.Fail ();
...
//	}
	...
	public void HasDiedOfOtherReasons(){
		if (this.intercepter.citizen.isDead) {
			//Citizen has died
			this.intercepter.sendReliefGoods.DeathByOtherReasons ();
			Task.current.Succeed();
		}else {
			Task.current.Fail ();
		}
	...
	private void Move(){
		if(this.intercepter.targetLocation != null){
			if(this.intercepter.path != null){
				if(this.intercepter.path.Count > 0){
					this.MakeCitizenMove (this.intercepter.location, this.intercepter.path [0]);
//					this.raider.daysBeforeMoving = this.raider.path [0].movementDays;
					this.intercepter.location = this.intercepter.path[0];
					this.intercepter.citizen.currentLocation = this.intercepter.path [0];
                    this.UpdateFogOfWar();
					this.intercepter.path.RemoveAt (0);
					this.intercepter.location.CollectEventOnTile(this.intercepter.citizen.city.kingdom, this.intercepter.citizen);
...
//					this.raider.daysBeforeMoving -= 1;
    ...
    private void CheckForKingdomDiscovery() {
		...
		tilesToCheck.Add(this.intercepter.location);
        tilesToCheck.AddRange(visibleTiles);
        for (int i = 0; i < tilesToCheck.Count; i++) {
            HexTile currTile = tilesToCheck[i];
            if (currTile.isOccupied && currTile.ownedByCity != null &&
				currTile.ownedByCity.kingdom.id != this.intercepter.citizen.city.kingdom.id) {
				Kingdom thisKingdom = this.intercepter.citizen.city.kingdom;
				...
				if (otherKingdom.id != this.intercepter.citizen.city.kingdom.id) {
                    thisKingdom.DiscoverKingdom(otherKingdom);
                    otherKingdom.DiscoverKingdom(thisKingdom);
                }
            }
    ...
    private void UpdateFogOfWar(bool forDeath = false) {
        for (int i = 0; i < visibleTiles.Count; i++) {
			...
			this.intercepter.citizen.city.kingdom.SetFogOfWarStateForTile(currTile, FOG_OF_WAR_STATE.SEEN);
        }
        visibleTiles.Clear();
        if (!forDeath) {
			visibleTiles.Add(this.intercepter.location);
			visibleTiles.AddRange(this.intercepter.location.AllNeighbours);
            for (int i = 0; i < visibleTiles.Count; i++) {
                HexTile currTile = visibleTiles[i];
				this.intercepter.citizen.city.kingdom.SetFogOfWarStateForTile(currTile, FOG_OF_WAR_STATE.VISIBLE);
            }
        }
	...
	void OnMouseEnter(){
		if (!UIManager.Instance.IsMouseOnUI()) {
			UIManager.Instance.ShowSmallInfo (this.intercepter.sendReliefGoods.name);
			this.HighlightPath ();
		}
	}
    ...
    private void Update() {
        if (KingdomManager.Instance.useFogOfWar) {
			if (this.intercepter.location.currFogOfWarState == FOG_OF_WAR_STATE.VISIBLE) {
    ...
    void HighlightPath(){
		...
		for (int i = 0; i < this.intercepter.path.Count; i++) {
			this.intercepter.path [i].highlightGO.SetActive (true);
			this.pathToUnhighlight.Add (this.intercepter.path [i]);
		}
	}
	...
	public void OnEndAttack(){
		this.intercepter.sendReliefGoods.DoneCitizenAction(this.intercepter.citizen);
		this.intercepter.DestroyGO ();
	}
