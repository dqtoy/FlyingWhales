...
public class RelieverAvatar : MonoBehaviour {
	public Reliever reliever;
...
//		}
//	}
	internal void Init(Reliever reliever){
		this.reliever = reliever;
		this.direction = DIRECTION.LEFT;
		this.GetComponent<Avatar> ().kingdom = this.reliever.citizen.city.kingdom;
		this.GetComponent<Avatar> ().gameEvent = this.reliever.sendReliefGoods;
		this.GetComponent<Avatar> ().citizen = this.reliever.citizen;
...
//	}
	void OnTriggerEnter2D(Collider2D other){
		if(other.tag == "Avatar"){
			if(this.gameObject != null && other.gameObject != null){
				if(other.gameObject.GetComponent<Avatar>().kingdom.id != this.reliever.citizen.city.kingdom.id){
			...
			if(this.gameObject != null && other.gameObject != null){
				Kingdom kingdomOfGeneral = this.reliever.citizen.city.kingdom;
...
//	}
	...
	public void IsThereEvent(){
		if(this.reliever.sendReliefGoods != null){
			Task.current.Succeed ();
		}else{
			Task.current.Fail ();
		}
	...
	public void HasArrivedAtTargetHextile(){
		if(this.reliever.location == this.reliever.targetLocation){
			if(!this.hasArrived){
				this.hasArrived = true;
				this.reliever.Attack ();
			}
			Task.current.Succeed ();
		}else{
			Task.current.Fail ();
...
//	}
	...
	public void HasDiedOfOtherReasons(){
		if (this.reliever.citizen.isDead) {
			//Citizen has died
			this.reliever.sendReliefGoods.DeathByOtherReasons ();
			Task.current.Succeed();
		}else {
			Task.current.Fail ();
		}
	...
	private void Move(){
		if(this.reliever.targetLocation != null){
			if(this.reliever.path != null){
				if(this.reliever.path.Count > 0){
					this.MakeCitizenMove (this.reliever.location, this.reliever.path [0]);
//					this.raider.daysBeforeMoving = this.raider.path [0].movementDays;
					this.reliever.location = this.reliever.path[0];
					this.reliever.citizen.currentLocation = this.reliever.path [0];
                    this.UpdateFogOfWar();
					this.reliever.path.RemoveAt (0);
					this.reliever.location.CollectEventOnTile(this.reliever.citizen.city.kingdom, this.reliever.citizen);
...
//					this.raider.daysBeforeMoving -= 1;
    ...
    private void CheckForKingdomDiscovery() {
		...
		tilesToCheck.Add(this.reliever.location);
        tilesToCheck.AddRange(visibleTiles);
        for (int i = 0; i < tilesToCheck.Count; i++) {
            HexTile currTile = tilesToCheck[i];
            if (currTile.isOccupied && currTile.ownedByCity != null &&
				currTile.ownedByCity.kingdom.id != this.reliever.citizen.city.kingdom.id) {
				Kingdom thisKingdom = this.reliever.citizen.city.kingdom;
				...
				if (otherKingdom.id != this.reliever.citizen.city.kingdom.id) {
                    thisKingdom.DiscoverKingdom(otherKingdom);
                    otherKingdom.DiscoverKingdom(thisKingdom);
                }
            }
    ...
    private void UpdateFogOfWar(bool forDeath = false) {
        for (int i = 0; i < visibleTiles.Count; i++) {
			...
			this.reliever.citizen.city.kingdom.SetFogOfWarStateForTile(currTile, FOG_OF_WAR_STATE.SEEN);
        }
        visibleTiles.Clear();
        if (!forDeath) {
			visibleTiles.Add(this.reliever.location);
			visibleTiles.AddRange(this.reliever.location.AllNeighbours);
            for (int i = 0; i < visibleTiles.Count; i++) {
                HexTile currTile = visibleTiles[i];
				this.reliever.citizen.city.kingdom.SetFogOfWarStateForTile(currTile, FOG_OF_WAR_STATE.VISIBLE);
            }
        }
	...
	void OnMouseEnter(){
		if (!UIManager.Instance.IsMouseOnUI()) {
			UIManager.Instance.ShowSmallInfo (this.reliever.sendReliefGoods.name);
			this.HighlightPath ();
		}
	}
    ...
    private void Update() {
        if (KingdomManager.Instance.useFogOfWar) {
			if (this.reliever.location.currFogOfWarState == FOG_OF_WAR_STATE.VISIBLE) {
    ...
    void HighlightPath(){
		...
		for (int i = 0; i < this.reliever.path.Count; i++) {
			this.reliever.path [i].highlightGO.SetActive (true);
			this.pathToUnhighlight.Add (this.reliever.path [i]);
		}
	}
	...
	public void OnEndAttack(){
		this.reliever.sendReliefGoods.DoneCitizenAction(this.reliever.citizen);
		this.reliever.DestroyGO ();
	}
