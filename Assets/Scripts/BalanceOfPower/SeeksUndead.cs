...
public static class SeeksUndead {
	...
	private static void Phase1(Kingdom kingdom, bool skipPhase2, bool skipPhase3, bool skipPhase4, bool hasAllianceInWar){
		Debug.Log("========== " + kingdom.name + " is seeking undead " + GameManager.Instance.month.ToString() + "/" + GameManager.Instance.days.ToString() + "/" + GameManager.Instance.year.ToString() + " ==========");
		Debug.Log ("========== PHASE 1 ==========");      
		...
		if(!skipPhase2){
			...
			if(!skipPhase3){
				...
				Debug.Log("==========SKIPPED PHASE 3 END SEEKS UNDEAD " + kingdom.name + " ==========");
			}
		}
			...
			if(!hasLeftAlliance && warsToJoin.Count > 0){
				for (int i = 0; i < warsToJoin.Count; i++) {
					...
					if(warsToJoin[i].warfare.IsAdjacentToEnemyKingdoms(kingdom, warsToJoin[i].side)){
						...
						if(warsToJoin [i].side == WAR_SIDE.A){
							oppositeSide = WAR_SIDE.B;
						}
						List<Kingdom> oppositeKingdoms = warsToJoin [i].warfare.GetListFromSide (oppositeSide);
						Kingdom targetKingdom = oppositeKingdoms [UnityEngine.Random.Range (0, oppositeKingdoms.Count)];
						targetKingdom.CreateSubterfugeEvent (SUBTERFUGE_ACTIONS.SPREAD_PLAGUE, targetKingdom);
						Debug.Log(kingdom.name + " decided to spread plague in " + targetKingdom.name);
					}
				}
				skipPhase4 = true;
			}
	...
	private static void Phase3(Kingdom kingdom, bool skipPhase2, bool skipPhase3, bool skipPhase4, bool hasAllianceInWar){
		...
		Debug.Log("========== END SEEKS UNDEAD " + kingdom.name + " ==========");
	}
}
