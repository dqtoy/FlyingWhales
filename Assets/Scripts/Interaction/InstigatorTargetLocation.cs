...
public class InstigatorTargetLocation : Interaction {
    ...
    private void SetAttackers() {
        ...
        //    if (resident.forcedInteraction == null && resident.doNotDisturb <= 0 && resident.IsInOwnParty() && !resident.isLeader && resident.role.roleType != CHARACTER_ROLE.CIVILIAN && !resident.currentParty.icon.isTravelling && !resident.isDefender && resident.specificLocation.id == areaToAttack.id) {
