...
public class SpawnCharacter : Interaction {
    ...
    private void StartEffect(InteractionState state) {
        WeightedDictionary<AreaDefenderSetting> classWeights = LandmarkManager.Instance.GetDefaultDefenderWeights(interactable.tileLocation.areaOfTile.owner.race);
        _classNameToBeSpawned = classWeights.PickRandomElementGivenWeights().className;

        state.descriptionLog.AddToFillers(null, Utilities.NormalizeString(interactable.tileLocation.areaOfTile.owner.race.ToString()), LOG_IDENTIFIER.STRING_1);
    ...
    private void SuccessCancelEffect(InteractionState state) {
        ...
        state.AddLogFiller(new LogFiller(null, Utilities.NormalizeString(interactable.tileLocation.areaOfTile.owner.race.ToString()), LOG_IDENTIFIER.STRING_1));
    ...
    private void FailCancelEffect(InteractionState state) {
        Character createdCharacter = CharacterManager.Instance.CreateNewCharacter(_classNameToBeSpawned, interactable.tileLocation.areaOfTile.owner.race, Utilities.GetRandomGender(), interactable.tileLocation.areaOfTile.owner, interactable);
