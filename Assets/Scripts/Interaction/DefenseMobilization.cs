...
public class DefenseMobilization : Interaction {
    ...
    private DefenderGroup CreateNewDefenderGroupFromIdleCharactersInArea() {
        ...
        for (int i = 0; i < interactable.tileLocation.areaOfTile.areaResidents.Count; i++) {
            ...
            if (resident.role.roleType != CHARACTER_ROLE.LEADER && resident.role.roleType != CHARACTER_ROLE.CIVILIAN && !resident.currentParty.icon.isTravelling && !resident.isDefender && resident.specificLocation.tileLocation.areaOfTile.id == interactable.tileLocation.areaOfTile.id) {
