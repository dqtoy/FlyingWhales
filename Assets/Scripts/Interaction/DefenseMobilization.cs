...
public class DefenseMobilization : Interaction {
    ...
    private DefenderGroup CreateNewDefenderGroupFromIdleCharactersInArea() {
        ...
        for (int i = 0; i < interactable.tileLocation.areaOfTile.areaResidents.Count; i++) {
            ...
            if (resident.IsInOwnParty() && !resident.isLeader && !resident.isDefender && !resident.currentParty.icon.isTravelling && resident.role.roleType != CHARACTER_ROLE.CIVILIAN && resident.specificLocation.tileLocation.areaOfTile.id == interactable.tileLocation.areaOfTile.id) {
