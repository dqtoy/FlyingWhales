...
public class GiftBeast : Interaction {
    ...
    public override void CreateStates() {
        _beast = _characterInvolved.currentParty.characters[1];
        ...
        startStateDescriptionLog.AddToFillers(_beast, _beast.name, LOG_IDENTIFIER.CHARACTER_3);
    ...
    private void GiftSuccessEffect(InteractionState state) {
        ...
        state.descriptionLog.AddToFillers(_beast, _beast.name, LOG_IDENTIFIER.CHARACTER_3);
        ...
        _characterInvolved.currentParty.RemoveCharacter(_beast);
        //_beast.RemoveTrait("Packaged");
        _beast.ChangeFactionTo(_targetCharacter.faction);
        _beast.MigrateHomeTo(_targetCharacter.homeArea);
        ...
        if (_characterInvolved.faction.id != _targetCharacter.faction.id && !_characterInvolved.isFactionless && !_targetCharacter.isFactionless) {
            ...
            throw new System.Exception("CAN'T DO GIFT BEAST: " + _characterInvolved.name + " of " + _characterInvolved.faction.name + " to " + _targetCharacter.name + " of " + _targetCharacter.faction.name);
        }
    }
