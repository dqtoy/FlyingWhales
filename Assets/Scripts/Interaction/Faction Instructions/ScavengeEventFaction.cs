...
public class ScavengeEventFaction : Interaction {
    ...
    private void DoNothingEffect(InteractionState state) {
        ...
        if (structure.structureType == STRUCTURE_TYPE.WAREHOUSE) {
            if (interactable.suppliesInBank > 0) {
    ...
    private void SupplyScavengedSuccessRewardEffect(InteractionState state) {
        int obtainedSupply = 0;
        //**Mechanics**: If the Structure is a Dungeon, randomize from the Dungeon's Supply Pile range.
        if (structure.structureType == STRUCTURE_TYPE.DUNGEON) {
            obtainedSupply = Random.Range(interactable.dungeonSupplyRangeMin, interactable.dungeonSupplyRangeMax + 1);
        } else if (structure.structureType == STRUCTURE_TYPE.WAREHOUSE) {
            //**Mechanics**: If the Structure is a Warehouse, randomize from 1 to the total amount of Supply in the Warehouse.
            obtainedSupply = Random.Range(1, interactable.suppliesInBank);
