...
public class DiplomatTargetLocation : Interaction {
    ...
    private void SetExpander() {
        ...
        for (int i = 0; i < interactable.tileLocation.areaOfTile.areaResidents.Count; i++) {
            ...
            if (resident.forcedInteraction == null && resident.doNotDisturb <= 0 && resident.IsInOwnParty() && !resident.isLeader && !resident.isDefender && !resident.currentParty.icon.isTravelling && resident.specificLocation.tileLocation.areaOfTile.id == interactable.tileLocation.areaOfTile.id && _targetLocationToken.location.possibleOccupants.Contains(resident.race)) {
