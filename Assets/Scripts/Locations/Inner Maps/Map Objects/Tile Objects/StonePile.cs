        ...
        if (gridTileLocation != null) {
            if (structureLocation.settlementLocation != null && structureLocation.settlementLocation.mainStorage == structureLocation) {
                if (resourceInPile < 100) {
                    if (structureLocation.settlementLocation != null && !structureLocation.settlementLocation.HasJob(JOB_TYPE.PRODUCE_STONE)) {
                        GoapPlanJob job = JobManager.Instance.CreateNewGoapPlanJob(JOB_TYPE.PRODUCE_STONE, new GoapEffect(GOAP_EFFECT_CONDITION.PRODUCE_STONE, string.Empty, false, GOAP_EFFECT_TARGET.ACTOR), this, structureLocation.settlementLocation);
