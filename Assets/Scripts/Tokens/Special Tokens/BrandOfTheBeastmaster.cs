...
public class BrandOfTheBeastmaster : SpecialToken {
    ...
    public override bool CanBeUsedBy(Character sourceCharacter) {
        //return true;
        return sourceCharacter.gender == GENDER.MALE && sourceCharacter.characterClass.attackType == ATTACK_TYPE.PHYSICAL && sourceCharacter.role.roleType != CHARACTER_ROLE.BEAST;
    }
    #endregion

    private void ItemUsedEffectMinion(TokenInteractionState state) {
        Character targetCharacter = state.target as Character;
        targetCharacter.ChangeClass("Beastmaster");
        ...
        Log log = new Log(GameManager.Instance.Today(), "Tokens", GetType().ToString(), state.name.ToLower() + "_special1", state.interaction);
    ...
    private void ItemUsedEffectNPC(TokenInteractionState state) {
        state.tokenUser.ChangeClass("Beastmaster");
        state.tokenUser.ConsumeToken(this);
