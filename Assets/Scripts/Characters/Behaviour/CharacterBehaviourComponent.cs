...
public abstract class CharacterBehaviourComponent {
    ...
    public bool CanDoBehaviour(Character character) {
        if(character.specificLocation.region.area.areaMap == null && HasAttribute(BEHAVIOUR_COMPONENT_ATTRIBUTE.INSIDE_SETTLEMENT_ONLY)) { //character.specificLocation.region.area.areaMap - will be changed after specificLocation rework
            return false;
        }else if (character.specificLocation.region.area.areaMap != null && HasAttribute(BEHAVIOUR_COMPONENT_ATTRIBUTE.OUTSIDE_SETTLEMENT_ONLY)) {
            return false;
        }
        return true;
    }
    ...
    public void PostProcessAfterSucessfullDoBehaviour(Character character) {
