...
public abstract class CharacterBehaviourComponent {
    ...
    public bool CanDoBehaviour(Character character) {
        if(character.currentSettlement == null && HasAttribute(BEHAVIOUR_COMPONENT_ATTRIBUTE.INSIDE_SETTLEMENT_ONLY)) { //character.specificLocation.region.settlement.areaMap - will be changed after specificLocation rework
            ...
            return false;
        }else if (character.currentSettlement != null && HasAttribute(BEHAVIOUR_COMPONENT_ATTRIBUTE.OUTSIDE_SETTLEMENT_ONLY)) {
