...
public class CharacterClassManager : MonoBehaviour {
    ...
    public Dictionary<string, Func<Character, string>> classIdlePlans { get; private set; }
    ...
    private Func<Character, string> defaultClassIdlePlan;

    public void Initialize() {
        ConstructAllClasses();
        ConstructClassIdlePlans();
    }
    ...
    private void ConstructClassIdlePlans() {
        //TODO:
        classIdlePlans = new Dictionary<string, Func<Character, string>>();
        defaultClassIdlePlan = DefaultClassIdlePlan;
    }
    public string RunIdlePlanForCharacter(Character character) {
        ...
        if (classIdlePlans.ContainsKey(className)) {
            return classIdlePlans[className](character);
        } else {
            return defaultClassIdlePlan(character);
        }
    }
    private string DefaultClassIdlePlan(Character character) {
        ...
        if (character.faction.id != FactionManager.Instance.neutralFaction.id) {
            ...
            if (character.previousCurrentActionNode != null && character.previousCurrentActionNode.action.goapType == INTERACTION_TYPE.RETURN_HOME && character.currentStructure == character.homeStructure) {
                ...
                character.PlanIdleReturnHome();
                return log;
            } else if (character.specificLocation != character.homeArea && character.trapStructure.structure == null) {
            ...
            if (!character.isAtHomeRegion) {
                log += "\n-" + character.name + " is in another area";
                log += "\n-100% chance to return home";
                character.PlanIdleReturnHome();
                return log;
            } else {
                log += "\n-" + character.name + " is in home area";
